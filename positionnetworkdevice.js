...
class positionnetworkdevice  {
    ...
    compute(){
        ...
        for (var s in this.list) {
            if (this.list[s].style.position){
                //positionnement spécifique
                switch (this.list[s].style.position.type){
                    case "relative":
                        //recherche de l'élément
                        var net = this.base.getInfra().find(this.list[s].style.position.class, this.list[s].style.position.name);
                        console.log('elkement relatif',this.base.getInfra(),net,this.list[s].style.position.class,this.list[s].style.position.name);
                        //application des transformers
                        var newpos = new THREE.Vector3()
                        newpos.copy(net.mainobj.position);
                        if (this.list[s].style.position.top){
                            newpos.z+=this.list[s].style.position.top;
                        }
